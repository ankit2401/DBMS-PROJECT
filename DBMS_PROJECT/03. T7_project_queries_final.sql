...
select distinct T1.T7_Cust_Id,concat(T1.T7_Cust_FName,T1.T7_Cust_LName) as T7_Cust_Name,T1.T7_Cust_MOB_Number,
	T2.T7_Vehicle_Id,T2.T7_Vehicle_Registration_Number,T2.T7_Vehicle_Manufacturer,T2.T7_Vehicle_Model_Number from T7_CUSTOMER 
    as T1 INNER JOIN T7_VEHICLE as T2 on T2.T7_Cust_id = T1.T7_Cust_Id where T2.T7_Vehicle_Id not in 
...
select * from T7_INSURANCE_COMPANY where T7_Company_Name in (select T7_Company_Name from T7_OFFICE group by T7_Company_Name 
    ...
    Count(distinct(T7_Product_Number))>Count(distinct(T7_Office_Name))) and count(distinct T7_Address)>1 );
